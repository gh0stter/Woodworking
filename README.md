...
## 2. 安全防护
* 护目镜
	原则上操作任何电动工具都需要戴, 但事实上很多人都会先麻烦不戴, 这个请大家执行斟酌, 电动工具很容易带出高速物体, 一不小心是会瞎眼的, 购买的时候一定买大牌子, 比如 3M, 否则等于没防护.
	对于戴眼镜的朋友, 可以用专门的罩在眼镜外边的护目镜, 比如下面这种:
	![Eye Protection](assets/eyeprotection.png)

* 噪音防护
	 要认准隔音耳罩的标准，对照自己电动工具的噪音水平，购买具有足够保护能力的耳罩。Noise Reduction Rating (NRR) 是 30 分贝，换算一下也就是可以减少环境噪音 11.5 分贝。换言之, 如果电锯的噪音是 100 分贝,  那么带上耳罩之后, 你听到的噪音就是 88.5 分贝. 
	
	 ![Hearing Protection](assets/hearingprotection.png)

* 粉尘/毒气防护
	粉尘是玩木头的主要主要污染源, 电锯、打磨、开槽等等都会产生粉尘，如果工房的除尘设备满足不了产生源除尘的话, 口罩是一定要戴的, 而且建议是 N95 级别的口罩.
	化学保护, 主要是刷漆或者涂装的时候, 现在大部分漆都是水性漆, 毒性已经减少了很多, 涂装的时候我建议使用低挥发, 低毒性的油 (桐油, 木蜡油等), 带上防毒面具能提供最大的保护. 
	
	![Respirator](assets/respirator.png)
	...
	![Dust Collection](assets/dustcollection.png)
	 
	 有条件的话, 建议安装一个专用的空气净化器, 这样工作的时候以及停工之后空气中的粉尘很快就会被吸滤, 避免粉尘一直飘荡在空气中, 或者最终落到地板和工具上, 变成污染源. 
	
	![Air Filter](assets/airfilter.png)

* 手脚防护
	玩木头的时候会风刀很多刀类, 这时候手套能很好的帮你保护不被割伤, 但是这里需要注意的一点是, 在使用旋转电动工具/机械的时候, 除非你很清楚你在做什么, 否则请不要使用手套, 如果手套不小心被卷入的话, 你的手会跟着一起卷入, 从而造成更大的伤害.
	脚的防护主要是劳保鞋, 鞋头有金属保护不被砸伤, 踢伤, 钉伤
	
	![Steel-toe boot](assets/steeltoeboot.png)

## 3. 中式，日式，欧式 (西式)
这世上手工工具主要有三个流派，流派的选择主要会影响你对工具的选择，间接影响在工具上的投入，我个人是欧式和日式结合。
* 中式
	第一印象就是中式刨子和框锯, 小时候印象最深的就是那个长者牛角的孢子, 还有木工师傅坐在板凳上刨木头
	![中式工具](assets/Chinese_Tools.png)
* 日式
	日式是由中式演化而来, 最主要的特点是"拉 (pull)", 例如拉刨, 拉锯. 还有可能是受到武士刀的影响, 一般会觉得他们的钢材性能比较好, 好友的就是用起来舒服
	
	![日式工具](assets/Japanese_Tools.png)
* 欧式 (西式)
	...
	![欧式工具](assets/Western_Tools.png)

	![欧刨结构](/assets/Western_Plane_Structure.png)

## 4. 主要入门工具的选择
这部分主要分为手工工具和电动 (机械) 工具的选择, 主要讲入门级别的选择, 更高级的选择和更详细的介绍会在具体的分类里面进行说明.
...
* 手工工具
	...
	* 锤子
		...
		* 雕刻锤, 这种一般是雕刻用的锤子, 一般为木制或者铜制, 圆筒形, 除非你要玩雕刻, 否则几乎不会用到
		...
		![Carving Mallet](assets/CarvingMallet.png)
		...
		* 橡胶锤, 这种锤子在组装工件的时候特别有用, 敲击木材的时候不会对木头造成损伤
		...
		![Rubber MAllet](assets/RubberMallet.png)
		
		* 日本锤, 又称玄能锤, 一般是用来敲击凿子, 调整刨刀用, 由于锤头是金属头, 就看个人喜好和配套工具是什么了
		
		![Japanese Hammer](assets/JapaneseHammer.png)
		...
		* 小 (铜) 锤, 这种小锤子一般是用来调整刨刀刀片的, 一般材质是脚软的金属 (铜) 或者木头, 这样在敲击刀片的时候不会对刀片造成伤害, 如果玩的是欧刨的话几乎不会用到这样的锤子, 因为绝大部分台刨 (Bench plane) 都带有平行调节机构 (当然也有例外), 如果是玩日本刨或者中式刨, 甚至是有调整需求的可以考虑买一个
		...
		![STERLING PLANE HAMMER](assets/StearingPlaneHammer.png)
	...
	* 锯子
		...
		这种几乎不会有二手的, 不缺钱的可以买 Knew Concepts 的曲线锯, 手头紧的可以去淘宝买高仿
		
		![Knew Concepts Coping Saw](assets/KnewConceptsCopingSaw.png)

* 电动工具/木工机械
	* 电钻
		这个基本上算是 DIY 必备了, 不管是不是玩木头, 只要在家装点东西维修点东西什么的，都会买一套电钻吧. 在购买电钻的时候最好是买电池的, 至于什么品牌取决于你的喜好, 一般 Milwaukee, Dewalt, Makita 三选一. 在购买的时候很多人会分不清 Impact Driver (冲击钻) 和 Drill (电钻), 这两个经常会放在一起卖. Impact Driver 一般是建筑用得比较多, 可以轻松打螺丝进木头. Drill (电钻) 顾名思义就是钻孔, 也可以拿来上螺丝, 很多都带有离合器, 适合扭矩有要求的场景, 不容易过度用力
		做家具的话基本上不会用到 Impact Driver, 直接打螺丝进硬木的话基本上是会开裂的, 软木会好很多, 但是也是可能会裂开, 正确的操作方法是先钻孔再上螺丝, 如果想省钱的话单买 Drill 即可.
		
		![Drill Set](assets/DrillSet.png)
	* 台钻
		如果有能力的话建议买一台 12 寸的台钻, 最好是有无级变速的. 台钻的好处在于有更高的精度, 做大量重复性打孔任务很方便
		
		![12 inch Drill Press](assets/drillpress.png)
	* 电木铣
		英文是 Router, 主要是用来铣槽, 倒角, 还有各种各样的花边, 能让你玩出很多花样, 但相对的这个工具也是比较危险的, 需要注意手不要靠近刀头, 铣的方向等等, 建议先看下面三个视频在决定是否要入手.
			- [WATCH THIS before you buy a router! (A lot of stuff you may not know!](https://www.youtube.com/watch?v=_1NNk8p8ca4)
			- [Routers - 7 Important Things You Should Know](https://www.youtube.com/watch?v=VJgSxZaBqg4)
			- [Router Safety](https://www.youtube.com/watch?v=dfj8k7lrIH8)
		根据夹头的大小一般分为两种, 1/4 inch 和 1/2 inch (公制地区能买到 6mm 和 12mm), 刚开始的话先买个 1/4 inch 的 router 电池或者非电池都可, 1/4 inch 夹头的一半只考虑 palm router/laminate trimmer router, 这种是一只手能抓起的大小 (当然使用的时候最好是双手), 使用起来很便利, 这时候如果是电池版的话会更方便, 但也更贵.
		
		![Router](assets/router.png)
	* 打磨机
		俗话说玩木头最讨厌的事情就是打磨, 有一台打磨机会让你的人生变得舒服点, 这里的打磨机特指 random orbital sander, 顾名思义, 这种打磨机会以随机 (假) 圆的方式旋转进行打磨, 这样就不太会在打磨件上面留下很深的固定划痕.
